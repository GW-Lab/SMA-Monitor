...
Public Class DataLog
   ...
   Public Function Values(t1 As Date, t2 As Date, Optional resolution As Integer = 600) As DataLogCSVJSON
            ...
            Case "humidity", "rain", "solarintensity", "temp_current", "winddirection", "windspeedMS"
               For Each item In obj.rv.Data
                  If item.Count = 1 Then
                     item.Add(prevValue)     ' if Only dateTimeStamp is present then add the 0 value  
                  End If

                  prevValue = item(1)        ' Store previous value
               Next
            Case "airpressure", "T1", "T2", "-T1", "-T2", "gas", "water"
               For Each item In obj.rv.Data
                  If item.Count = 1 Then
                     item.Add(prevValue)
                  End If
      ...
      Catch ex As Exception
         '   MessageBox.Show($"Data error {ex.Message} {ex.StackTrace}")

         Return New DataLogCSVJSON
      End Try
   End Function
   Public Function Values(t1 As UInteger, t2 As UInteger, Optional resolution As Integer = 600) As DataLogCSVJSON
      Dim steps = 0
      Dim prevValue = 0.0!
